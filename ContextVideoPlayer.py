...
class myThread(threading.Thread):
    ...
    def run(self):
        ...
        with(lock):
            shutil.copyfile(os.path.abspath('zee.jpg'),os.path.abspath('zee1.jpg'))

        img = processImage('zee1.jpg')
        if img is not None:
            Globals.Paused = True
            sleep(.5)
            videoWindow.config(image=img)
    ...
    if (not Globals.Stopped):
        if (not Globals.Paused):
            ...
            try:
                ...
                videoWindow.after(100, show_frame)
                return
            seekBar.set(sliderValue)
            ...
            videoWindow.image = img
            videoWindow.configure(image=img)
        else:
        ...
        videoWindow.after(10, show_frame)
        pass
    else:
        videoWindow.after(100, show_frame)
...
videoWindow = ttk.Label(root)
videoWindow.grid(row=0, column=0, columnspan=3, rowspan=1, sticky=(N, S, E, W))

seekBar = ttk.Scale(root, orient=HORIZONTAL, from_=1.0, to=100.0, command=setFrame)
seekBar.grid(row=1, column=0, columnspan=3, rowspan=3, sticky=(N, S, E, W))
...
myfiletypes = [('Video files', '.mp4 .avi')]
